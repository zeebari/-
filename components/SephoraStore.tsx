...
const PAYMENT_METHODS: PaymentMethod[] = [
  {
    ...
    name: 'QiCard',
    color: '#1a6fb5',
    info: '1570214617 / 07724576000',
    details: {
      ar: 'رقم الكارت: 1570214617\nرقم الموبايل: 07724576000',
      en: 'Card No: 1570214617\nMobile: 07724576000',
      ku: 'ژمارەی کارت: 1570214617\nژمارەی موبایل: 07724576000',
    },
  },
  {
    id: 'fib',
    ...
    info: '07504381396',
    details: {
      ar: 'رقم الحساب: 07504381396',
      en: 'Account: 07504381396',
      ku: 'ژمارەی حساب: 07504381396',
    },
  },
  {
    id: 'fastpay',
  ...
  },
  {
    ...
    info: '07504381396',
    details: {
      ar: 'رقم المحفظة: 07504381396',
      en: 'Wallet: 07504381396',
      ku: 'ژمارەی جزدان: 07504381396',
    },
  },
  {
    id: 'asiahawala',
    ...
    info: '07724576000',
    details: {
      ar: 'رقم الحساب: 07724576000',
      en: 'Account: 07724576000',
      ku: 'ژمارەی حساب: 07724576000',
    },
  },
  {
    id: 'cihan',
    ...
    info: '020034132401',
    details: {
      ar: 'رقم الحساب: 020034132401\nرقم الموبايل: 07724576000',
      en: 'Account: 020034132401\nMobile: 07724576000',
      ku: 'ژمارەی حساب: 020034132401\nژمارەی موبایل: 07724576000',
    },
  },
  {
    id: 'cash',
...
const T = {
  ...
  en: {
    ...
    orderTotal: (total: number, pm: PaymentMethod) =>
      `\n💰 Total: ${total.toLocaleString()} IQD\n\n💳 Payment: ${pm.id === 'cash' ? 'Cash on delivery' : pm.name}\n${pm.details.en}`,
    dir: 'ltr' as const,
  },
  ku: {
    ...
    sendOrder: 'داواکاری بنێرە لە واتساپ',
    payWith: 'پارەدان بە',
    backToCart: '← گەڕانەوە بۆ سەبەتە',
    emptyCart: 'سەبەتەکەت بەتاڵە',
    cashName: 'کاش لە کاتی وەرگرتن',
    orderHeader: '🛍️ داواکاری نوێ — سیفۆرا کوردستان',
    orderLine: (name: string, qty: number, price: number) =>
      `• ${name} × ${qty} = ${price.toLocaleString()} د.ع`,
    orderTotal: (total: number, pm: PaymentMethod) =>
      `\n💰 کۆی گشتی: ${total.toLocaleString()} د.ع\n\n💳 شێوازی پارەدان: ${pm.id === 'cash' ? 'کاش لە کاتی وەرگرتن' : pm.name}\n${pm.details.ku}`,
    dir: 'rtl' as const,
  },
}
...
export default function SephoraStore() {
  ...
  const openCart = () => { setPaymentStep(false); setSelectedPayment(null); setCartOpen(true) }
  const closeCart = () => { setCartOpen(false); setPaymentStep(false); setSelectedPayment(null) }
  ...
  const handleCheckout = () => {
    ...
    const msg = `${t.orderHeader}\n\n${lines.join('\n')}${t.orderTotal(cartTotal, selectedPayment)}`
  ...
  return (
    <div style={{ background: BG, minHeight: '100vh', color: '#fff', direction: t.dir }}>
      ...
      <div style={{
        ...
        borderBottom: `1px solid ${GOLD_DIM}`, position: 'relative', overflow: 'hidden',
      }}>
        <div style={{ position: 'absolute', inset: 0, pointerEvents: 'none', background: 'radial-gradient(ellipse 60% 50% at 50% 100%, #C9A84C18 0%, transparent 70%)' }} />
        <div style={{ position: 'relative', maxWidth: 560, margin: '0 auto' }}>
          ...
          <p style={{ color: '#888', fontSize: '0.95rem', lineHeight: 1.7, margin: 0 }}>{t.heroSub}</p>
      ...
      {cartOpen && (
        <>
          <div onClick={closeCart} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.72)', zIndex: 100, backdropFilter: 'blur(3px)' }} />
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '1.1rem 1.25rem', borderBottom: `1px solid ${BORDER}` }}>
              <h2 style={{ margin: 0, color: GOLD, fontWeight: 800, fontSize: '1rem' }}>
                {paymentStep ? `💳 ${t.choosePayment}` : `🛍️ ${t.cart}`}
              </h2>
              <button onClick={closeCart} style={{
                background: 'transparent', border: `1px solid ${BORDER}`, borderRadius: 6,
                width: 30, height: 30, color: '#666', cursor: 'pointer', fontSize: '0.9rem',
            ...
            {!paymentStep ? (
                        ...
                        return (
                          <div key={item.product.id} style={{ background: SURFACE2, borderRadius: 10, padding: '0.875rem', border: `1px solid ${BORDER}` }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.5rem', marginBottom: '0.6rem' }}>
                              ...
                              <button onClick={() => removeItem(item.product.id)} style={{ background: 'transparent', border: 'none', color: '#444', cursor: 'pointer', fontSize: '0.8rem', flexShrink: 0, padding: 2 }}>🗑️</button>
                            </div>
                ...
                {cart.length > 0 && (
                  <div style={{ padding: '1.1rem 1.25rem', borderTop: `1px solid ${BORDER}` }}>
                    ...
                    <button onClick={() => setPaymentStep(true)} style={{
                      width: '100%', background: GOLD, border: 'none', borderRadius: 10,
                      padding: '13px 0', color: '#000', fontSize: '0.95rem', fontWeight: 800,
              ...
              <>
                <div style={{ flex: 1, overflowY: 'auto', padding: '1rem 1.25rem' }}>
                  {/* Back link */}
                  <button onClick={() => { setPaymentStep(false); setSelectedPayment(null) }} style={{
                    background: 'transparent', border: 'none', color: '#666',
                    cursor: 'pointer', fontSize: '0.8rem', padding: '0 0 1rem',
                    ...
                    {PAYMENT_METHODS.map(pm => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            background: isSelected ? `${pm.color}22` : SURFACE2,
                            border: `1px solid ${isSelected ? pm.color : BORDER}`,
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                            <div style={{
                              ...
                              background: pm.id === 'cash' ? '#2a2a2a' : pm.color,
                              display: 'flex', alignItems: 'center', justifyContent: 'center',
                              fontSize: pm.id === 'cash' ? '1.2rem' : '0.65rem',
                              color: '#fff', fontWeight: 800, letterSpacing: '-0.03em',
                            }}>
                              {pm.id === 'cash' ? '💵' : pm.name.substring(0, 3).toUpperCase()}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: 700, fontSize: '0.88rem', color: isSelected ? '#fff' : '#ccc' }}>
                                {pm.id === 'cash' ? t.cashName : pm.name}
                              </div>
                              {pm.info && (
                                <div style={{ fontSize: '0.7rem', color: '#666', marginTop: 2 }}>{pm.info}</div>
                              )}
                            </div>
                            <div style={{
                              width: 18, height: 18, borderRadius: '50%', flexShrink: 0,
                              border: `2px solid ${isSelected ? pm.color : '#333'}`,
                              background: isSelected ? pm.color : 'transparent',
                              display: 'flex', alignItems: 'center', justifyContent: 'center',
                            }}>
                ...
                <div style={{ padding: '1.1rem 1.25rem', borderTop: `1px solid ${BORDER}` }}>
                  ...
                  <button
                    ...
                    style={{
                      width: '100%',
                      background: selectedPayment ? 'linear-gradient(135deg, #25D366, #128C7E)' : '#1a1a1a',
                      border: 'none', borderRadius: 10, padding: '13px 0',
                      color: selectedPayment ? '#fff' : '#444',
                  ...
                  >
                    <WhatsAppIcon active={!!selectedPayment} />
                    {t.sendOrder}
                  </button>
                </div>
              </>
      ...
      <footer style={{ textAlign: 'center', padding: '2rem 1.25rem', borderTop: `1px solid ${GOLD_DIM}`, color: '#444', fontSize: '0.78rem' }}>
        ...
        <div>Original beauty products from UAE</div>
      </footer>
    </div>
  )
}
...
function ProductCard({ product, iqd, lang, inCart, addToCart, addToCartLabel }: {
  product: Product; iqd: number; lang: Lang; inCart: number
  addToCart: (p: Product) => void; addToCartLabel: string
}) {
  ...
  return (
    ...
    >
      <div style={{
        ...
        <div style={{ fontSize: '0.55rem', letterSpacing: '0.2em', color: `${GOLD}88`, textTransform: 'uppercase' }}>{product.brand}</div>
      ...
      <div style={{ padding: '0.875rem' }}>
        <div style={{ fontSize: '0.58rem', color: '#555', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 3 }}>{product.brand}</div>
        <div style={{ fontSize: '0.85rem', fontWeight: 600, lineHeight: 1.4, minHeight: '2.4em', marginBottom: '0.75rem' }}>{product.name[lang]}</div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '0.5rem', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => addToCart(product)} style={{
            ...
            border: `1px solid ${GOLD}`, color: inCart > 0 ? '#000' : GOLD,
...
function QtyBtn({ onClick, children }: { onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{
      width: 28, height: 28, background: '#1e1e1e', border: `1px solid ${BORDER}`,
      borderRadius: 6, color: '#fff', cursor: 'pointer', fontSize: '1rem',
      display: 'flex', alignItems: 'center', justifyContent: 'center',
    }}>
      {children}
    </button>
...
function WhatsAppIcon({ active }: { active: boolean }) {
  return (
    <svg width="18" height="18" viewBox="0 0 24 24" fill={active ? 'white' : '#444'}>
